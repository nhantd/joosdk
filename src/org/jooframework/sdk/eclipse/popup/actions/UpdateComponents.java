...
public class UpdateComponents implements IObjectActionDelegate, IAdaptable {
	...
	public void run(IAction action) {
		...
		Job job = new Job("Update components") {
			@Override
			protected IStatus run(IProgressMonitor monitor) {
				try {
					if (res instanceof IContainer) {
						JooSDK.update((IContainer)res);
					} else {
						JooSDK.update((IFile)res);
					}
				} catch (Exception ex) {
