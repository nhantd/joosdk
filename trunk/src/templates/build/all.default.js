...
/*ImageObject = DisplayObject.extend	({
	...
	setupBase: function(config) {
		this.className = this.className || "ImageObject";
		this._super(config);
	},
	
	setupDomObject: function(config) {
		this._super(config);
		this.access().attr('src', config.src);
	},
...
RotatableImage = ImageObject.extend({
	...
	rotate: function(e)	{
		...
		parent.setStyle('-moz-transform', 'rotate('+deg+'deg)');
		parent.setStyle('-webkit-transform', 'rotate('+deg+'deg)');
		parent.setStyle('transform', 'rotate('+deg+'deg)');
	}
});
*/

/*
Input = DisplayObject.extend({
	
	setupDomObject: function(config) {
		this._super(config);
		this.access().val(config.value);
		this.access().attr('name', config.name);
	},
...
TextInput = Input.extend({
	...
	setupBase: function(config) {
		this.className = this.className || "TextInput";
		this._super(config);
	},
...
*/
ContextMenu = DisplayObjectContainer.extend({
	setupBase: function(config)	{
	...
	setupDomObject: function(config) {
		this._super(config);
		this.access.css('z-index', config.zIndex || 10);
	}
});
...
ImageBox = Sketch.extend({
	...
	getZIndex: function()	{
		if(this.zIndex == null){
			this.zIndex = this.setStyle("z-index");
		}
		if(this.zIndex == null){
			this.zIndex = 1000;
		}
